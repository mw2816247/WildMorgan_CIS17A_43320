...
class Array {
    protected:
        unsigned short size;
        T *content;
        virtual void newAry(short); //Classes that inherit may have their own rules for initializing new Arrays
    public:
        Array(short s) {
            newAry(s);
        }
        Array() {
            content = nullptr; //If no information is provided, can't size or populate an array
            //BUT we can set content to nullptr! If the destructor is called afterwards,
...
template <class T>
void Array<T>::dupSort() { ////This function is an edit of code from Mike Kitchen.
    //His original code can be found on https://www.codeproject.com/Articles/8657/A-Simple-Yahtzee-Game
